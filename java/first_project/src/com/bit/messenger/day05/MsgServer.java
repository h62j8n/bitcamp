...
public class MsgServer extends Frame implements Runnable {
	...
	public MsgServer(){
		...
		mbList.add(new Member("test2", "2222", "테스트계정2"));
		mbList.add(new Member("test3", "3333", "테스트계정3"));
		
//		System.out.println(mbList.get(2).getId());
	...
	public void SvrStart(){
		...
		try {
			...
			while(true){
				...
				System.out.println("스레드 생성 전 "+mbList.get(2).getId());
