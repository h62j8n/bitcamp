...
public class LoginUI extends Frame {
	String ip="192.168.0.10"; //67:박준서 29:조혜진
	int port = 5000;
	...
	public void msgParsing(String msg) {
		...
		switch (job) {
			...
			case Member.CHAT_CR: //채팅룸 만들기
				chatCreateResult();
				break;
			case Member.CHAT_MEMBER: //채팅룸 사용자 추가하기
				chatMemberList();
				break;
			case Member.CHAT_SYNC: //채팅방 대화내용 싱크 맞추기
				chatSync();
				break;
			case Member.CHAT_INVITE: //채팅방 초대를 받은경우 화면을 열어주자
				chatInviteResult();
				break;				
		}
	}
	...
	// #logout 
	// input Member.LOGOUT+"|"+mb.getId()+"|"+Member.MLO00+"|"
	...
	// #채팅방 싱크 맞추기
	// input: Member.CHAT_SYNC+"|"+chat.getRoomNum()+"|"+msg
	public void chatSync() {
		String strRoomNum = token.nextToken();
		int RoomNum = Integer.parseInt(strRoomNum);
		String msg = token.nextToken();
		
		for(int i=0; i<mb.getMbChatRooms().size();i++){
			if(RoomNum == mb.getMbChatRooms().get(i).getRoomNum()){
				//텍스트 커서 위치 조정
				mb.getMbChatRooms().get(i).getChatUI().ta1.setCaretPosition(mb.getMbChatRooms().get(i).getChatUI().ta1.getText().length());
				//메세지 싱크 
				mb.getMbChatRooms().get(i).getChatUI().ta1.append(msg+"\n");
			}//end of if
		}//end of for
	}//end of chatSync()
	...
	// #채팅방 개설 완료처리
	// input: Member.CHAT_CR+"|"+Member.OK+"|"+Member.MCC01+"|"+StrChatNo;
	public void chatCreateResult() {
		String result = token.nextToken();
		
		if (result.equals(Member.OK)) {
			String responseMsg 	= token.nextToken();
			String StrChatNo 	= token.nextToken();
			int chatNo = Integer.parseInt(StrChatNo);
			mu.createChat(chatNo, responseMsg); //MainUI의 채팅창 생성 함수를 호출
		}
	}
	
	// #채팅방 참여자목록 (서버 응답)
	public void chatMemberList() {
		String strRoomNum = token.nextToken();
		int RoomNum = Integer.parseInt(strRoomNum);
		...
		for(int i=0; i<mb.getMbChatRooms().size();i++){
			if(RoomNum == mb.getMbChatRooms().get(i).getRoomNum()){
//				mb.getMbChatRooms().get(i).getChatUI().chatMbList.clear(); 
				myRoom = mb.getMbChatRooms().get(i); 
				
			}//end of if
		}//end of for
		
		List myList = myRoom.getChatUI().chatMbList;
		myList.clear();
		while(token.hasMoreElements()){
			String mbId = token.nextToken();
			String mbName = token.nextToken();
			myList.add(mbName+" ("+mbId+")");
	...
	// #채팅방 초대자 화면 오픈 처리 
	// input: Member.CHAT_IV+"|"+Member.id(초대자)+"|"+Member.MIV01+"|"+StrChatNo;
	public void chatInviteResult() {
		String id = token.nextToken();
		String responseMsg 	= token.nextToken();
		String StrChatNo 	= token.nextToken();
		
		String msg = id+"님으로 부터 "+ responseMsg;
		int chatNo = Integer.parseInt(StrChatNo);
		mu.invitedChat(chatNo, msg); //MainUI의 채팅창 생성 함수를 호출
	...
	public void idCheckResult() {
		String result = token.nextToken();
		Color unEnable = new Color(230, 230, 230);
		
		if (result.equals(Member.OK)) {
			sysMsgSignIn.setText(Member.MID00);
			btnIdDup.setText("확인 완료");
			btnIdDup.setBackground(unEnable);
...
//			cliInOutSettingReq(); //클라이언트 고유의 io 셋팅값을 서버로 전송시켜서 기억시키자.
			...
			submit(Member.FR_LIST+"|"+mb.getId());			// 친구목록 요청
			mu.myInform.setText(name+" ("+id+") 님의 친구목록");
			mu.sysMsg.setText(name+"님, 환영합니다.");
	...
	public void frendListResult() {
		...
		if (result.equals(Member.OK)) {
			mu.friendTable.setNumRows(0);
			while(token.hasMoreElements()){
				Member fr = new Member();
				String id = token.nextToken();
				String name = token.nextToken();
				String phone = token.nextToken();
				String online = token.nextToken();
				...
				fr.setPhone(phone);
				frList.add(fr);
				String[] frConts = {online, name, id};
				mu.friendTable.addRow(frConts);
				mb.setFrList(frList);
			}
		} else {
			// TODO 친구가 없을 경우
//			mu.friendList.add("등록된 친구가 없습니다.");
	...
	public void findFriendResult() {
		...
		if (result.equals(Member.OK)) {
			while (token.hasMoreElements()) {
				...
				String[] schConts = {name, id, phone};
				mu.searchTable.addRow(schConts);
			}
	...
	public void friendAddResult() {
		String result = token.nextToken();
		if (result.equals(Member.OK)) {
			mu.sysMsgSearch.setText(Member.MAD00);
			submit(Member.FR_LIST+"|"+mb.getId());			// 친구목록 요청
			System.out.println("friendAddResult() 추가한 회원 : "+mb.getId());
		} else {
			// TODO 친구등록 실패
		}
	}
	...
	// #친구삭제 결과 (서버 응답)
	public void friendRemoveResult() {
		String result = token.nextToken();
		if (result.equals(Member.OK)) {
			mu.sysMsg.setText(Member.MRV00);
			submit(Member.FR_LIST+"|"+mb.getId());			// 친구목록 요청
		} else {
			// TODO 친구삭제 실패
		}
