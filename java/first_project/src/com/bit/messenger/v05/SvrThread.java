...
//		System.out.println(tempList.size());
	...
	private void chatSyncMsg(String chatNo, String msg){
		for(int i=0; i<chatList.size(); i++){
			if(Integer.parseInt(chatNo) == chatList.get(i).getRoonNum()){
	...
	private void chatSyncMsg(ChatRoom chat, String msg){
		for(int i=0; i<chat.getRoomMemberList().size(); i++){
			chat.getRoomMemberList().get(i).getPs().println(msg);
			sysMsg(chat.getRoonNum()+"번 채팅방의 메시지 ["+msg+"]가 " +chat.getRoomMemberList().size()+"개의 채팅창에 전달되었습니다.");
		}
	}
		...
		for(int i=0; i<chatList.size(); i++){
			...
			if(Integer.parseInt(chatNo) == chatList.get(i).getRoonNum()){
				while(token.hasMoreTokens()){
					...
					//입장완료 메시지를 보내자! 
					String msg = mb.getName()+"("+mb.getName()+")님이 입장하셨습니다.";
					chatSyncMsg(chatList.get(i), msg);
				}//end of while(token)
