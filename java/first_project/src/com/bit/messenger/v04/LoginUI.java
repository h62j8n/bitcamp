...
public class LoginUI extends Frame {
	...
	// #UI 경고 다이얼로그
	public Dialog alertUi(String msg) {
		final Dialog d = new Dialog(frame, true);
		int dW = 400;				// dialogWidth
		int dH = 200;				// dialogHeight
//		int inW = 400;				// innerWidth
//		int inCenX = (dW-inW)/2;	// innerCenter X (가운데 정렬)
		
		Image img = kit.createImage("images\\ico_question.png");
		Icon icon = new ImageIcon(img);
		
		JLabel qMark = new JLabel(icon);
		Label qText = new Label(msg);
		Panel pQuest = new Panel(null);
		pQuest.add(qMark);
		pQuest.add(qText);
		
		qMark.setBounds(0, 0, 50, 50);
		qText.setBounds(50, 0, 150, 50);
		pQuest.setLocation(200, 50);
